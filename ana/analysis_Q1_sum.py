...
list_path = ["Final_Result_test1/ROB_"+ str(ROB)+"_final_result.txt", "Final_Result_test3/ROB_"+ str(ROB)+"_final_result.txt", "Final_Result_test5/ROB_"+ str(ROB)+"_final_result.txt", "Final_Result_test6/ROB_"+ str(ROB)+"_final_result.txt"]
dataset =["data1", "data3-1", "data3-2", "data3-3"]
ax = plt.axes(xlim=[4,40], ylim=[0,12], xlabel=r'Q$_{1}$', ylabel='# of events [/0.1]') #r"$\\frac{{\\chi^2}}{{\\text{{NDF}}}}"
for i, ipath in enumerate(list_path):
    ...
    plt.hist(df["Q_{1}"], range=xrange, bins=bins, label ="Q$_{1}$: mean = %.1f, std = %.1f" %(y_mean, y_std))
    plt.plot(bin_centers, gaussian(bin_centers, *popt), 'r-', linewidth = 2,  label=r"fit : $\mu$ = %.1f, $\sigma$ = %.1f" %(popt[1], popt[2]))  # Plotting the fitted curve
...
plt.show()
ax = plt.axes(xlim=[0,63], ylim=[-40,40], xlabel=r'channel', ylabel='Ratio [%]') #r"$\\frac{{\\chi^2}}{{\\text{{NDF}}}}"
...
plt.title("ROB "+ str(ROB))
plt.legend()
plt.savefig("ROB_"+ str(ROB)+"_Q1_relativevsChannel1.pdf")
plt.show()

ax = plt.axes(xlim=[0,63], ylim=[0,40], xlabel=r'channel', ylabel='Q$_{1}$') #r"$\\frac{{\\chi^2}}{{\\text{{NDF}}}}"
...
plt.title("ROB "+ str(ROB))
plt.legend()
plt.savefig("ROB_"+ str(ROB)+"_Q1vsChannel1.pdf")
plt.show()
