...
for i in range(8):
    ...
    ax.scatter(
        ...
        marker="o",
        c="blue",
        label="data 1: mean = %.1f, std = %.1f" % (y1_mean, y1_std),
    ...
    ax.scatter(
        ...
        marker="o",
        c="red",
        label="data 3: mean = %.1f, std = %.1f" % (y3_mean, y3_std),
    )  # Plotting the histogram
    ...
    ax2.scatter(
        ...
        marker="+",
        c="darkgreen",
        label="Relative ratio : mean = %.1f, std = %.1f" % (y_mean, y_std),
    )  # Plotting the histogram
    ax2.fill_between(
        x=df1["Channel"], y1=y_min, y2=y_max, color="pink", interpolate=True, alpha=0.2
    )
    ...
    plt.savefig("Result/pedestal_check" + str(i) + ".pdf")
    plt.show()
