...
for ROB in [7]:
    ...
    path4  = "../Result_test4/"+ filename
    #Create a PdfPages object to save figures into a PDF file
    pdf_pages = PdfPages('TT_ROB' + str(ROB) +'_abnormal_waveform.pdf')
    ...
    for i in channellist:
        ...
        with uproot.open(path6) as file:
                ...
                sigma = df5.iloc[i,7]
                if i < 10:
                    hist6 = "h_charge_ROB0" +str(ROB)+"_ch0"+str(i)+"_sigma_"+str(sigma)
                else:
                    hist6 = "h_charge_ROB0" +str(ROB)+"_ch"+str(i)+"_sigma_"+str(sigma)
        ...
        plt.hist(bin_edges[:-1], bins=bin_edges,linewidth=2,  weights=normalized_bin_contents1,histtype ="step", label="data 1")  # Plotting histogram using bin edges and values
        plt.hist(bin_edges3[:-1], bins=bin_edges3, linewidth=2, weights=normalized_bin_contents3,histtype ="step",label="data 3-1")  # Plotting histogram using bin edges and values
        plt.hist(bin_edges5[:-1], bins=bin_edges5, linewidth=2,weights=normalized_bin_contents5,histtype ="step",label="data 3-2")  # Plotting histogram using bin edges and values
        plt.hist(bin_edges6[:-1], bins=bin_edges6, linewidth=2, weights=normalized_bin_contents6,histtype ="step",label="data 3-3")  # Plotting histogram using bin edges and values
