...
df = pd.read_csv("Final_Result_test/ROB_"+ str(ROB)+"_final_result.txt", sep="\t",engine="python")
print(df)
# print(df["Q_{1}"])
...
ax.scatter(df["Channel"], df["Chi2NDF"] , marker = '*', c="red", alpha =0.5, label =r"$\frac{\chi^2}{NDF}$")  # Plotting the histogram
...
plt.legend()
plt.savefig("Final_Result_test/ROB_"+ str(ROB)+"_Q1vsChannel1.pdf")
plt.show()
...
ax = plt.axes(xlim=[20,120], ylim=[0,10], xlabel=r'Q$_{1}$', ylabel='# of events [/0.1]') #r"$\\frac{{\\chi^2}}{{\\text{{NDF}}}}"
...
plt.title("ROB "+ str(ROB))
plt.legend()
plt.savefig("Final_Result_test/ROB_"+ str(ROB)+"_Q1_mean.pdf")
plt.show()
...
plt.savefig("Final_Result_test/ROB_"+ str(ROB)+"_Q1vsChannel2.pdf")
plt.show()
